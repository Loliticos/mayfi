...
module.exports = class Npm extends Command {
  ...
  async run ({ channel, author, t, language }, npmPackage) {
    ...
    embed
      ...
      .addField(t("commands:npm.lastPublish"), `**v${pkg[0].version}** ${t("commands:npm.publisher")} **${pkg[0].publisher.username}** ${t("commands:npm.date")} **${moment(pkg[0].date).format('LLL')}**\n(${moment(pkg[0].date).fromNow()})`)
    channel.send(embed)
