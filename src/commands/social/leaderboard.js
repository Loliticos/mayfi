...
module.exports = class Leaderboard extends Command {
  ...
  async run ({ channel, guild, author, t, prefix }, text) {
    ...
    if(text.toLowerCase() == "level") {
      ...
      points = topToCheckPoints.splice(0, 5)
    }
    ...
    embed
      ...
      .setDescription(`
        **1.** __${this.client.users.get(top[0]._id)} (${this.client.users.get(top[0]._id).tag})__\n**${t(`commands:leaderboard.${lowerCased == "reps" ? "reps" : "level"}`)}**: ${top[0][lowerCased == "reps" ? "reps" : "level"]} ${text == "level" ? `- **${t("commands:leaderboard.points")}**: ${points[0].exp}` : ""}\n
        **2.** __${this.client.users.get(top[1]._id)} (${this.client.users.get(top[1]._id).tag})__\n**${t(`commands:leaderboard.${lowerCased == "reps" ? "reps" : "level"}`)}**: ${top[1][lowerCased == "reps" ? "reps" : "level"]} ${text == "level" ? `- **${t("commands:leaderboard.points")}**: ${points[1].exp}` : ""}\n
        **3.** __${this.client.users.get(top[2]._id)} (${this.client.users.get(top[2]._id).tag})__\n**${t(`commands:leaderboard.${lowerCased == "reps" ? "reps" : "level"}`)}**: ${top[2][lowerCased == "reps" ? "reps" : "level"]} ${text == "level" ? `- **${t("commands:leaderboard.points")}**: ${points[2].exp}` : ""}\n
        `)
    channel.send(embed)
